...
from flashsim.model.video_dit.alpadreams import CosmosDiT, CosmosDiTConfig, CosmosDiTCache, CosmosDiTCondition
...
def create_mock_data(
    ...
    x = x + torch.arange(num_views, device=device, dtype=dtype)[None, :, None, None, None, None]

    # Condition mask [B, V, 1, T, H, W] - for first frame conditioning
    ...
    text_embeddings = torch.randn(batch_size, num_views, 512, 100352, device=device, dtype=dtype)

    # Timestep [B]
    timesteps = torch.full((batch_size,), 500.0, device=device, dtype=dtype)

    # HDMap condition [B, V, C, T, H, W]
    if encode_with_pixel_shuffle:
        hdmap_condition = torch.randn(batch_size, num_views, 192, T, H, W, device=device, dtype=dtype)
    else:
        hdmap_condition = torch.randn(batch_size, num_views, 16, T, H, W, device=device, dtype=dtype)

    image = torch.randn(batch_size, num_views, 16, 1, H, W, device=device, dtype=dtype)
...
def test_alpadreams(
    ...
    # patchify the data
    image = model.patchify(data_dict["image"])
    hdmap = model.patchify(data_dict["hdmap_condition"])
    condition_video_input_mask_first_block = model.patchify(data_dict["condition_video_input_mask_first_block"])
    condition_video_input_mask_other_blocks = model.patchify(data_dict["condition_video_input_mask_other_blocks"])
