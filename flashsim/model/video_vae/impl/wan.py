            ...
            if parallel:
                ...
                if feat_cache is None:
                    ...
                    if rest.shape[2] > 0:
                        ...
                        rest_up = self._upsample_time(
                            rest_up, b, c, rest.shape[2], h, w
                        )
...
class WanVAE_(nn.Module):
            ...
            for j in range(0, width, self.tile_sample_stride_width):
                ...
                for k in range(frame_range):
                    ...
                    tile = self.encoder(
                        ...
                        if cache is None
                        else cache.enc_feat_map,
                        feat_idx=self._enc_conv_idx
                        if cache is None
                        else cache.enc_conv_idx,
                    )
            ...
            for j in range(0, width, tile_latent_stride_width):
                ...
                if self.parallel_decode:
                    ...
                    if cache is None:
                        ...
                        decoded = self.decoder(
                            tile,
                            feat_cache=cache.dec_feat_map,
                            feat_idx=cache.dec_conv_idx,
                            parallel=True,
                        )
                    row.append(decoded)
                else:
                    ...
                    for k in range(num_frames):
                        ...
                        decoded = self.decoder(
                            ...
                            feat_cache=self._feat_map
                            if cache is None
                            else cache.dec_feat_map,
                            feat_idx=self._conv_idx
                            if cache is None
                            else cache.dec_conv_idx,
                        )
                        time.append(decoded)
                    row.append(torch.cat(time, dim=2))
            rows.append(row)
    ...
    def encode(self, x, scale, return_mu=False, cache: Optional[WanVAECache] = None):
        if cache.enc_feat_map[0] is not None:  # not the first chunk
            ...
            for i in range(iter_):
                x_chunk = x[:, :, i * 4 : (i + 1) * 4, :, :]
                out = self.encode_single_chunk(x_chunk, scale, cache)
                results.append(out)
    ...
    def decode(self, z, scale, cache: Optional[WanVAECache] = None):
        ...
        if self.parallel_decode:
            ...
            if cache is None:
                self._conv_idx = [0]
                out = self.decoder(
                    x, feat_cache=self._feat_map, feat_idx=self._conv_idx, parallel=True
                )
            else:
                cache.dec_conv_idx = [0]
                out = self.decoder(
    ...
    def sample(
        ...
        deterministic=False,
        scale=[0, 1],
        cache: Optional[WanVAECache] = None,
    ):
...
class WanVAE:
    def __init__(
        self,
        z_dim=16,
        vae_path="lighttaew2_1.safetensors",
        dtype=torch.float,
    ...
    def encode_dist(
        ...
        if self.use_tiling:
            encoded_chunk = self.model.tiled_encode(
                video_chunk, self.scale, cache=cache
            )
        else:
        ...
        if self.parallel:
            ...
            if self.use_2d_split:
                ...
                if width % world_size == 0:
                    out = self.encode_dist(
                        video, world_size, cur_rank, split_dim=4, cache=cache
                    )
                elif height % world_size == 0:
                    out = self.encode_dist(
                        video, world_size, cur_rank, split_dim=3, cache=cache
                    )
                else:
    ...
    def decode_dist(
        self, zs, world_size, cur_rank, split_dim, cache: Optional[WanVAECache] = None
    ):
        ...
        if cur_rank == 0:
            ...
            images = torch.narrow(
                images, split_dim, images.shape[split_dim] - pixel_chunk, pixel_chunk
            ).contiguous()
        else:
            images = torch.narrow(
                images, split_dim, 8 * padding_size, pixel_chunk
            ).contiguous()
        ...
        if self.parallel:
            ...
            if self.use_2d_split:
                ...
                if latent_width % world_size == 0:
                    images = self.decode_dist(
                        zs, world_size, cur_rank, split_dim=zs.ndim - 1, cache=cache
                    )
                elif latent_height % world_size == 0:
                    images = self.decode_dist(
                        zs, world_size, cur_rank, split_dim=zs.ndim - 2, cache=cache
                    )
                else:
                    logger.info("Fall back to naive decode mode")
                    images = self.model.decode(zs, self.scale, cache=cache).clamp_(
                        -1, 1
                    )
        else:
            decode_func = (
